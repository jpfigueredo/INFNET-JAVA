...
public class Guerreiro extends Personagem{
	...
	public Guerreiro(int forca, int resistencia, int defesa, List<String> meleeSkills) throws NumeroNegativoException, NumeroNuloException, SkillsInvalidasException, AtributosExcedidosException {
		...
		if(forca < 0 || resistencia < 0 || defesa < 0) {throw new NumeroNegativoException("Habilidades não podem ser negativas.");}
		if(forca > 10 || resistencia > 10 || defesa > 10) {throw new AtributosExcedidosException("Atributos excedidos. Nível máximo de atributos: 10.");}
		if(forca == 0 || resistencia == 0 || defesa == 0) {throw new NumeroNuloException("Habilidades não podem ser iguais à 0.");}
		if(meleeSkills.isEmpty()) {throw new SkillsInvalidasException("Habilidades inválidas, insira uma habilidade.");}
